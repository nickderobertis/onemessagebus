...
describe("the npm distribution", () => {
  ...
  before(() => {
    ...
    run("cargo", ["build", "--locked", "--quiet", "-p", "onemessagebus-cli"]);

    work = mkdtempSync(join(tmpdir(), "onemessagebus-npm-"));
    dist = join(work, "dist");
  ...
  it("puts a working `onemessagebus` on PATH", () => {
  ...
  it("propagates the binary's exit code rather than its own", () => {
    ...
    // docs/cli.md assigns exit 2 to input a verb refuses, and a caller branches
    // on it; a launcher that collapsed every failure to 1 would hide the
    // contract's own codes. A stream file that is not there is the cheapest way
    // to reach one.
    const refused = launch(project, ["events", "merge", "no-such-stream.ndjson"]);
    assert.equal(refused.code, 2, refused.stderr);
    assert.match(refused.stderr, /no-such-stream\.ndjson/);

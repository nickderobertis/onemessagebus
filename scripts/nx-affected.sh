...
base_branch() {
  local ref="${ONEMESSAGEBUS_NX_BASE_REF:-${GITHUB_BASE_REF:-}}"
  if [ -z "$ref" ]; then
    if [ -n "${CI:-}" ]; then
      ...
      echo "nx-affected: not a pull-request build, so every project runs (set ONEMESSAGEBUS_NX_BASE_REF to scope one)" >&2
      return 1
    fi
    printf 'main'
    return 0
  ...
  if ! printf '%s' "$ref" | grep -Eq '^[A-Za-z0-9][A-Za-z0-9._/-]*$'; then
    echo "nx-affected: '$ref' is not a usable branch name — set ONEMESSAGEBUS_NX_BASE_REF (or GITHUB_BASE_REF) to a plain one such as 'main'" >&2
    return 1
  fi
  printf '%s' "$ref"
}

...
No product's vocabulary or schema is compiled into this crate: a program
declares its source words, reserved labels and dimensions in its own crate, and
`tests/vocabulary.rs` proves the core works over a vocabulary it declares
itself. A new wire fact belongs on the `Vocabulary` trait as a type or a
constant, never as a word in here.
...
`conformance.rs` is test support published for a program's vocabulary crate;
it is excluded from the coverage floor and exercised by its callers.
...
documents are held by `tests/contract.rs` against
